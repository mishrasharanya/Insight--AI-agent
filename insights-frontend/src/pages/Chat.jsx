...
export default function Chat() {
  ...
  return (
    <div className="flex flex-col h-screen">
      <div className="border-b border-[#1a1a1a]/10 px-8 py-5">
...
// Custom markdown component renderers — styled to match the InsightAI theme
const markdownComponents = {
  h1: ({ node, children, ...props }) => (
    <h1 className="font-heading text-xl mt-4 mb-2 text-[#1a1a1a]" {...props}>
      {children}
    </h1>
  ),
  h2: ({ node, children, ...props }) => (
    <h2 className="font-heading text-lg mt-4 mb-2 text-[#1a1a1a]" {...props}>
      {children}
    </h2>
  ),
  h3: ({ node, children, ...props }) => (
    <h3 className="font-heading text-base mt-4 mb-1.5 text-[#1a1a1a] uppercase tracking-wide" {...props}>
      {children}
    </h3>
  ),
  p: ({ node, children, ...props }) => (
    <p className="text-sm leading-relaxed my-2 text-[#1a1a1a]" {...props}>
      {children}
    </p>
  ),
  ...
  em: ({ node, children, ...props }) => (
    <em className="italic text-[#1a1a1a]/80" {...props}>
      {children}
    </em>
  ),
  ul: ({ node, children, ...props }) => (
    <ul className="list-disc pl-5 my-2 space-y-1 text-sm text-[#1a1a1a]" {...props}>
      {children}
    </ul>
  ),
  ol: ({ node, children, ...props }) => (
    <ol className="list-decimal pl-5 my-2 space-y-1 text-sm text-[#1a1a1a]" {...props}>
      {children}
    </ol>
  ),
  ...
  code: ({ node, inline, children, ...props }) =>
    inline ? (
      <code
        className="px-1.5 py-0.5 bg-[#1a1a1a]/5 text-[#b8541f] font-mono text-[13px] rounded-sm"
        {...props}
      >
        {children}
      </code>
      ...
      <code
        className="block p-3 bg-[#1a1a1a]/5 text-[#1a1a1a] font-mono text-[13px] rounded-sm overflow-x-auto my-2"
        {...props}
      >
        {children}
      </code>
  ...
  blockquote: ({ node, children, ...props }) => (
    <blockquote
      className="border-l-2 border-[#b8541f] pl-4 my-2 italic text-[#1a1a1a]/80 text-sm"
      {...props}
    >
      {children}
    </blockquote>
  ...
  a: ({ node, children, ...props }) => (
    <a
      className="text-[#b8541f] underline underline-offset-2 hover:text-[#1a1a1a] transition-colors"
      target="_blank"
  ...
  thead: ({ node, children, ...props }) => (
    <thead className="border-b border-[#1a1a1a]/20" {...props}>
      {children}
    </thead>
  ),
  th: ({ node, children, ...props }) => (
    <th
      className="text-left px-3 py-2 font-semibold text-[#1a1a1a] text-xs uppercase tracking-wide"
      {...props}
    >
      {children}
    </th>
  ...
  td: ({ node, children, ...props }) => (
    <td className="px-3 py-2 border-b border-[#1a1a1a]/10 text-[#1a1a1a]" {...props}>
      {children}
    </td>
  ),
  hr: ({ node, ...props }) => (
    <hr className="my-4 border-[#1a1a1a]/10" {...props} />
  ),
};
...
function Message({ message }) {
  ...
  return (
    <div className="flex gap-4" data-testid={`chat-message-${message.role}`}>
      <div
        className={`w-8 h-8 flex items-center justify-center text-xs font-mono shrink-0 ${
          isUser ? "bg-[#1a1a1a]/10 text-[#1a1a1a]" : "bg-[#1a1a1a] text-[#f5f1e8]"
        }`}
      >
      ...
      <div
        className={`pt-1 flex-1 min-w-0 ${
          message.isError ? "text-red-600" : "text-[#1a1a1a]"
        }`}
      >
        ...
        {!isUser && message.tier && (
          <div className="mt-3 flex items-center gap-3">
            ...
            {message.route && (
              <span className="font-mono text-[10px] uppercase tracking-widest text-[#1a1a1a]/50">
                route: {message.route}
              </span>
            )}
          </div>
      ...
      <div className="flex gap-1">
        {[0, 1, 2].map((i) => (
          <div
            ...
            style={{ background: i < filled ? color : "#1a1a1a20" }}
          />
        ))}
      </div>
      <span className="font-mono text-[10px] uppercase tracking-widest text-[#1a1a1a]/50">
