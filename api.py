...
def get_current_user_id(request: Request):
    ...
    if not user_id:
        raise HTTPException(
            status_code=401,
            detail="Not logged in. Connect your Google account first.",
        )
...
def google_callback(code: str, request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Login session expired or invalid. Click Connect Google again.",
        )
    ...
    if not credentials.refresh_token:
        raise HTTPException(
            status_code=500,
            detail=(
                "Google did not return a refresh token. Try disconnecting this app "
                "from https://myaccount.google.com/permissions and connecting again."
            ),
        )
...
def picker_token(user_id: str = Depends(get_current_user_id)):
    ...
    if not developer_key:
        raise HTTPException(
            status_code=500,
            detail="GOOGLE_PICKER_API_KEY or GOOGLE_API_KEY is not configured.",
        )

    if not app_id:
        raise HTTPException(
            status_code=500,
            detail="GOOGLE_APP_ID is not configured.",
        )
...
def chat(request: ChatRequest, user_id: str = Depends(get_current_user_id_optional)):
    ...
    result = Planner.answer_with_planner(
        request.question,
...
def list_tabular_files(user_id: str = Depends(get_current_user_id_optional)):
    collection_name = (
        google_sync.collection_name_for(user_id)
        if user_id
        else ingest.COLLECTION_NAME
    )
...
def get_tabular_data(
    ...
    if not response.data:
        raise HTTPException(
            status_code=404,
            detail=f"No structured data found for '{filename}'.",
        )
...
def add_workspace(request: WorkspaceRequest):
    ...
    return {
        "success": True,
        "message": message,
        "folders": workspace.load_workspaces(),
    }
...
def purge_privacy_data(user_id: str = Depends(get_current_user_id_optional)):
    ...
    return privacy.purge_all_local_data(
        confirm=True,
        collection_name=collection_name,
    )
